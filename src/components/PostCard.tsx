...
export function PostCard({uid, src, title, date}:PostCardProps){
    ...
    return(
        ...
        className=" rounded-t-lg h-1/2 w-full "
            />
            <div
            className="w-full h-1/2 rounded-b-lg p-9 flex items-center flex-col"
            >
                <h1
            className="text-center font-semibold text-xl"
                ...
                <p
                className="mt-auto font-thin text-start w-full text-sm"
                >{d.toLocaleDateString('pt-br', {day:'2-digit',month: 'long', year:'numeric'})}</p>
            </div>
